...
const surgeryCapabilities = [
  { name: "Retinal microsurgery", detail: "iOCT · anatomy · tissue deformation → sound" },
  { name: "Percutaneous intervention", detail: "Tool–tissue interaction → sound" },
  { name: "Surgical navigation", detail: "Position · trajectory · spatial relationship → sound" },
] as const;

const scientificProof = [
  { name: "Years of research", detail: "Auditory interaction and data sonification for complex systems" },
  { name: "Multiple surgical domains", detail: "Microsurgery · navigation · intervention" },
  { name: "Peer-reviewed", detail: "Medical Image Analysis · MICCAI · IEEE · IPMI" },
...
export default function Homepage() {
  return <><SkipLink /><SiteHeader /><main id="main-content">
    ...
    <section id="surgery" className="section surgery-frontier" aria-labelledby="surgery-title"><Container><SectionLabel>First frontier</SectionLabel><div className="surgery-frontier__heading"><h2 id="surgery-title">Surgery.</h2><p>Auditory intelligence for image-guided intervention, physical interaction, and increasingly intelligent operating-room systems.</p></div><div className="surgery-frontier__visual" role="img" aria-label="Abstract image-guided intervention field"><div className="surgery-frontier__anatomy"><span /><span /><span /></div><div className="surgery-frontier__instrument" /><div className="surgery-frontier__signal"><i /><i /><i /></div></div><p className="surgery-frontier__evidence-line">Demonstrated across image-guided surgery, microsurgery, and tool–tissue interaction.</p><div className="surgery-frontier__capabilities">{surgeryCapabilities.map((item,index) => <article key={item.name}><span>0{index + 1}</span><h3>{item.name}</h3><p>{item.detail}</p></article>)}</div></Container></section>

    <section id="evidence" className="section scientific-foundation" aria-labelledby="scientific-foundation-title"><Container><SectionLabel>Scientific foundation</SectionLabel><div className="scientific-foundation__heading"><h2 id="scientific-foundation-title">Built through research.<br /><em>Designed for real systems.</em></h2><p>Sustained research, experimental demonstrations, peer review, and protected technology underpin the SoniXense platform.</p></div><div className="scientific-foundation__proof">{scientificProof.map((item,index) => <article key={item.name}><span>0{index + 1}</span><h3>{item.name}</h3><p>{item.detail}</p></article>)}</div></Container></section>
    ...
    <section id="contact" className="section closing-cta" aria-labelledby="closing-title"><Container><SectionLabel>Build with SoniXense</SectionLabel><h2 id="closing-title">Hear beyond vision.</h2><p>Bring auditory intelligence into your system.</p><ButtonLink href="mailto:contact@sonixense.com">Talk to us →</ButtonLink></Container></section>
  </main><SiteFooter /></>;
}
